...
const Home = () => {
    ...
    // Inizializza la classe dal localStorage se presente (solo per gli studenti)
    const [classeSelezionata, setClasseSelezionata] = useState(() => {
        if (isSegreteria) return "";
        return localStorage.getItem("classeSelezionata") || "";
    });
    ...
    // 2. Caricamento Lezioni
    const fetchLezioni = async () => {
        if (!classeSelezionata) return; // Nessuna classe selezionata, non fare chiamate

        // Otteniamo la sessione (se c'è). Se l'utente non è loggato, sarà null.
        const { data: { session } } = await supabase.auth.getSession();
        
        const url = `${import.meta.env.VITE_API_URL}/lezioni/?classe_id=${classeSelezionata}`;

        // Prepariamo gli header in modo condizionale
    ...
    // 3. Caricamento Opzioni Tendina (Solo Segreteria)
    useEffect(() => {
    ...
    const handleCambioClasse = () => {
        setClasseSelezionata("");
        localStorage.removeItem("classeSelezionata");
    };

    const handleSelezionaClasse = (id) => {
        setClasseSelezionata(id);
        if (!isSegreteria) {
            localStorage.setItem("classeSelezionata", id);
        }
    };
        ...
        return (
            <div className="page-container centered-page">
                <div className="auth-card" style={{ textAlign: 'center' }}>
                    <h2>{isSegreteria ? "Gestione Calendari" : "Seleziona la tua classe"}</h2>
                    <p className="text-muted" style={{ marginBottom: '20px' }}>
                        {isSegreteria ? "Seleziona una classe per gestire il calendario." : "Scegli la tua classe per visualizzare l'orario delle lezioni."}
                    </p>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'center' }}>
                        {classi.map(c => (
                            <button 
                                key={c.id} 
                                onClick={() => handleSelezionaClasse(c.id)}
                                className="login-btn"
                                style={{ minWidth: '80px', flex: '0 1 auto' }}
                            >
                                {c.nome}
                            </button>
                        ))}
            ...
            <div className="calendar-card">
                ...
                <div className="calendar-header">
                    <div className="title-group">
                        ...
                        <button onClick={handleCambioClasse} className="btn-link">
                            (Cambia classe)
                        </button>
                    </div>
